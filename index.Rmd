...
```{r, echo=FALSE}
normalized_pop=socioeconomic[,1,]%*%solve(diag(socioeconomic[1,1,]))
plot(1,1,xlim=c(0,5),ylim=c(0,4000),col=color[1],xlab="population (1950=1)",ylab="total plastic consumption (kt) (1950=1)",type = "l")
for (i in c(1:n_country)){
  points(socioeconomic[,1,i]/socioeconomic[1,1,i],consumption[,7,i]/consumption[1,7,i],col=color[i],type="l")
  text(socioeconomic[n_year,1,i]/socioeconomic[1,1,i],consumption[n_year,7,i]/consumption[1,7,i],dimnames(consumption)[[3]][i])
}
```
...
```{r, echo=FALSE}
normalized_consumption = t(consumption[,7,])/consumption[1,7,]
logYn = t(log1p(normalized_consumption))
